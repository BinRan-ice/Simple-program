...
void filling1(int c[], int d[], COLORREF col[MAX + 1][MAX + 1])
{
	...
	for (int i = 0; i <= MAX; i++)
	{
		...
		for (int j = 0; j <= MAX; j++)
		{
			d[(2 * j) + 1] = (2 * j) + 1;
			//用拉格朗日三次插值计算插点的像素值
			color2 = (d[(2 * j) + 1] - d[(2 * j)+2])*(d[(2 * j) + 1] - d[(2 * j) + 4])*(d[(2 * j) + 1] - d[(2 * j) + 6]) / (d[(2 * j)] - d[(2 * j) + 2])*(d[(2 * j)]   - d[(2 * j) + 4])*(d[(2 * j)] - d[(2 * j) + 6])   * (getpixel(d[(2 * j)], c[i]));
			color3 = (d[(2 * j) + 1] - d[(2 * j)])  *(d[(2 * j) + 1] - d[(2 * j) + 4])*(d[(2 * j) + 1] - d[(2 * j) + 6]) / (d[(2 * j)+2] - d[(2 * j)])  *(d[(2 * j)+2] - d[(2 * j) + 4])*(d[(2 * j)+2] - d[(2 * j) + 6]) * (getpixel(d[(2 * j) + 2], c[i]));
			color4 = (d[(2 * j) + 1] - d[(2 * j)])  *(d[(2 * j) + 1] - d[(2 * j) + 2])*(d[(2 * j) + 1] - d[(2 * j) + 6]) / (d[(2 * j)+4] - d[(2 * j)])  *(d[(2 * j)+4] - d[(2 * j) + 2])*(d[(2 * j)+4] - d[(2 * j) + 6]) * (getpixel(d[(2 * j) + 4], c[i]));
			color5 = (d[(2 * j) + 1] - d[(2 * j)])  *(d[(2 * j) + 1] - d[(2 * j) + 2])*(d[(2 * j) + 1] - d[(2 * j) + 4]) / (d[(2 * j)+6] - d[(2 * j)])  *(d[(2 * j)+6] - d[(2 * j) + 2])*(d[(2 * j)+6] - d[(2 * j) + 4]) * (getpixel(d[(2 * j) + 6], c[i]));
...
void filling2(int c[], int d[])
{
	...
	for (int i = 0; i <= 1000; i++)
	{
		...
		if (c[i] % 2 == 1)
		{
			...
			{
				//用拉格朗日三次插值计算插点的像素值
				color2 = (c[i]-  c[i + 1])*(c[i] - c[i + 3])*(c[i] - c[i + 5]) / (c[i - 1] - c[i + 1])*(c[i - 1] - c[i + 3])*(c[i - 1] - c[i + 5])* (getpixel(d[j], c[i - 1]));
				color3 = (c[i] - c[i - 1])*(c[i] - c[i + 3])*(c[i] - c[i + 5]) / (c[i + 1] - c[i - 1])*(c[i + 1] - c[i + 3])*(c[i + 1] - c[i + 5])* (getpixel(d[j], c[i + 1]));
				color4 = (c[i] - c[i - 1])*(c[i] - c[i + 1])*(c[i] - c[i + 5]) / (c[i + 3] - c[i - 1])*(c[i + 3] - c[i + 1])*(c[i + 3] - c[i + 5])* (getpixel(d[j], c[i + 3]));
				color5 = (c[i] - c[i - 1])*(c[i] - c[i + 1])*(c[i] - c[i + 3]) / (c[i + 5] - c[i - 1])*(c[i + 5] - c[i + 1])*(c[i + 5] - c[i + 3])* (getpixel(d[j], c[i + 5]));
				putpixel(d[j], c[i], col2[i / 2][j]);
			}
		}
	}
